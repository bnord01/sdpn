 ...
 *
 */
class MDPN2TA[GS <% HTR, StackSymbol <% HTR, DPNAction, Lock](dpn: MonitorDPN[GS, StackSymbol, DPNAction, Lock], val name: String = "cflow") extends DatalogTreeAutomata {
    ...
    def genScript = {
        ...
        for (state <- dpn.getControlSymbols)
            outln(name + "_state(" + state.toTerm + ").")
        for (stack <- dpn.getStackSymbols)
            outln(name + "_stack(" + stack.toTerm + ").")
...
name_term(1).
        ...
        outln("name_nil(t(G,S),c(G,S,G,0)) :- name_state(G), name_stack(S).".replace("name", name))
        ...
        def call2(fromState: GS, fromStack: StackSymbol, toState: GS, callStack: StackSymbol, returnStack: StackSymbol) {
            ...
            out("c(" + fromState.toTerm + "," + fromStack.toTerm + ", Gf,T))") //so we can annotate it with fromState,fromStack Gf and T
            outln(" :- name_state(Gc),name_state(Gf),name_term(T).".replace("name", name))
        }
        ...
        def call1(fromState: GS, fromStack: StackSymbol, toState: GS, callStack: StackSymbol) {
            ...
            out("c(" + fromState.toTerm + "," + fromStack.toTerm + ", Gc,0))") //so we can annotate it with fromState,fromStack Gc and 0
            outln(" :- name_state(Gc).".replace("name", name))
        }

        def spawn(fromState: GS, fromStack: StackSymbol, toState: GS, toStack: StackSymbol, spawnState: GS, spawnStack: StackSymbol) {
            out(name + "_spawn(")
            out("c(" + spawnState.toTerm + "," + spawnStack.toTerm + ",Gs,Ts ), ") //some thread has been spawned
            out("c(" + toState.toTerm + ", " + toStack.toTerm + ",Gf,T), ") //and then we reached Gf , T
            out("c(" + fromState.toTerm + "," + fromStack.toTerm + ", Gf,T))") //so we can annotate it with fromState,fromStack Gf and T
            outln(" :- name_state(Gs),name_term(Ts),name_state(Gf),name_term(T).".replace("name", name))
        }
        ...
        def base(fromState: GS, fromStack: StackSymbol, toState: GS, toStack: StackSymbol) {
            out(name + "_base(")
            out("c(" + toState.toTerm + "," + toStack.toTerm + ",Gf,T), " +
                "c( " + fromState.toTerm + "," + fromStack.toTerm + ",Gf,T))")
            outln(" :- name_state(Gf),name_term(T).".replace("name", name))
        }

        out("%All states beginning at the initial state + stack are final\n")
        out(name + "_final(")
        out("c(" + dpn.initialState.toTerm + "," + dpn.initialStack.toTerm + ",Gf,T))")
        outln(" :- name_state(Gf),name_term(T).".replace("name", name))
